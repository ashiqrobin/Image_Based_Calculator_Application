...
originalImage = cv2.imread("/Users/mertaydayanc/Desktop/imageCalculatorVenv/try1.png",0)
...
for i, ctr in enumerate(sorted_ctrs):
    ...
    if w > 5 and h > 5:
        cv2.rectangle(copy_orig,(x,y),(x+w,y+h),(0,255,0),2)
        cv2.imwrite('C:\\Users\\Link\\Desktop\\output\\{}.png'.format(i), roi)
        images.append(roi)

cv2.imshow('ROIs', copy_orig)
cv2.waitKey(0)
#binary
expression = []
for image1 in images:
   ...
   if(height > 10 or width >10):
      ...
      ret,thresh = cv2.threshold(image1,200,255,cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
      kernel = np.ones((3,3), np.uint8)
      img_dilation = cv2.dilate(thresh, kernel, iterations=1)
      #Add Border
      bordersize=2
      border=cv2.copyMakeBorder(img_dilation, top=bordersize + 5, bottom=bordersize + 5, left=bordersize, right=bordersize, borderType= cv2.BORDER_CONSTANT, value=[0,0,0] )
      img = cv2.resize(border, (28,28))
      cv2.imshow('img', img)
      cv2.waitKey(0)
...
for c in contours:
    ...
    if(h1 > 20 and w1 > 50):
       ...
       x,y,w,h = rect
       cv2.rectangle(originalImage,(x,y),(x+w-10,y+h),(0,255,0),2)
       cv2.putText(originalImage,resultStr,(x+w-250,y+h +20),0,0.6,(0,255,0))
